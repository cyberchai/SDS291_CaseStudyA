...
# plot(model_lvl, 1)
```
In this case, our data does not make a flat and horizontal reference line, thus it does not satisfy the linearity test, meaning *what does the linearity test prove*.
...
A linear relationship requires constant variance. While our residuals aren't scattered with constant variance across all fitted values, there is still a large amount of variance. Thus, *we can conclude...*
...
# plot(model_lvl, 2)
```
We see in the Normality of Residuals plot above that the data points for the sample quantile deviations vs the normal distribution quantiles do not make a horizontal line shape. *If they made a horizontal line shape...*

*We've completed 3 tests to determine how *
