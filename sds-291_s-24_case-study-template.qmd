...
ggplot(data = election_wo_pb, aes(x = Bush2000, y = Buchanan2000)) +
  geom_point() +
  ggtitle("Without Palm Beach") + scale_x_continuous(trans = "log10", labels = scales::comma) + geom_smooth(method = lm, formula = y~x) + xlab("Bush Votes (log)") + ylab("Buchanan Votes")
```
```{r}
...
There were 3407 votes for Buchanan, but our prediction interval tells us that the number of votes expected for Buchanan 95% of the time is between 364 and 830, so the likely number of votes intended for Gore but cast for Buchanan were between 3407-830 =  and 3407-364 =  
